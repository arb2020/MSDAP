...
wire [39:0] shiftIO;
    ...
    ALU_Controller ALU_Con(.rj(rj), .rj_address(rj_address), .coeff(coeff), .coeff_address(coeff_address), .x_address(x_address), .Sclk(Sclk), .en_ALU(en_ALU), .done(done), .Enable(Enable), .load(load), .opcode(opcode), .shift_en(shift_en), .feedbackLoad(feedbackLoad), .clear(clear), .Reset_in(Reset_n));
